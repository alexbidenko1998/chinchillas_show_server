...
{
    public function upload($chinchilla_id, Request $request)
    {
        $chinchilla = Chinchilla::where(['id' => $chinchilla_id, 'owner_id' => $request->user()->id])
            ->firstOr(function () {
        ...
        $request->file('photo')
            ->storeAs('chinchillas/' . $chinchilla->owner_id . '/' . $chinchilla_id . '/', $filename, 'public_photos');
    ...
    public function delete($photo_id, Request $request)
    {
        $photo = ChinchillaPhoto::findOrFail($photo_id);
        $chinchilla = Chinchilla::where(['id' => $photo->chinchilla_id, 'owner_id' => $request->user()->id])
            ->firstOr(function () {
                abort(Response::HTTP_FORBIDDEN);
            });

        Storage::disk('public_photos')
